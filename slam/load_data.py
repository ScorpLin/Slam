...
def polar2cart(scan, angles):
    ...
    z = np.zeros(len(scan))
    return np.vstack((x, y, z)), z


if __name__ == "__main__":
    joint_data = get_joint("data/train/train_joint0")
    lidar_data = get_lidar("data/train/train_lidar0")
#    
    test = lidar_data[0]['pose']
    lidar_scan = lidar_data[0]['scan']
    lidar_scan_1d = lidar_scan.ravel('F')
#    
#    
#    lidar_angles = np.linspace(start=-135*np.pi/180, stop=135*np.pi/180, num=1081).reshape(1,-1)
#    lidar_angles_1d = lidar_angles.ravel('F')
#    
...
#    x = lidar_scan[good_range]
#    y = lidar_angles[good_range]
#    lidar_hit, z = polar2cart(lidar_scan[good_range], lidar_angles[good_range]) # 3*n
    
    
#    yaw_bias = lidar_data[0]['rpy'][0,2]
#    pose_bias = lidar_data[0]['pose'][0,:2]
#    print('length: ', len(lidar_data))
#    print(yaw_bias)
#    print(pose_bias)
